...
# ── ★ GATE 8 — → is MATERIAL IMPLICATION (E12: → = ¬A∨B), the one connective no gate reached before 2026-09-10.
#  Its four classical rows over the world's own two atoms must read TFTT. An explicit PRESENCE pattern, because the
#  catch-all *FAIL* below cannot see a row that has stopped being printed at all.
case "$PROUT" in
  *"implication-is-material OK"*) : ;;
  *) echo "FAIL  prop: → is no longer material implication (E12: → = ¬A∨B) — row implication-is-material not OK, or missing — got: $PROUT"; ok=0 ;;
esac
# ── ★ GATE 9 — ∧ is CO-PRESENCE (⊕), commutative (LA :2845-:2852): the conjunction of the world's two atoms renders
#  as ⊕ and is one glyph in either operand order. A presence pattern, for the same reason as GATE 8's.
case "$PROUT" in
  *"and-is-copresence OK"*) : ;;
  *) echo "FAIL  prop: ∧ is no longer co-presence (⊕, commutative) — row and-is-copresence not OK, or missing — got: $PROUT"; ok=0 ;;
esac
...
#   stay opgrammar.la's.
# ★ 11 pairs share a glyph: 7 DECLARED, each with its citation in crosscoll.la
#   (Truth=SR_ABOUT and Truth=OP_RECOG by E16 §C; Compassion=OP_INTEG by E10),
#   and 4 OPEN,
...
#   - Past=Death is gone: the General ruled Death = ⊂(𝔤₆,𝔤₇), c67 (E16 §B′),
#     which forms no pair. Past keeps >76.
...
#   - Warm=OP_INTEG does not form here: E10 moved 𝔄 to ⊗(LOVE,RECOGNITION).
#   None of the four is a ruling. The pin exists so that a new collision, or
...
#   and :4806 AS RULED by E16 §B′ (Death = c67).
...
#   turns census FAIL at F=5. B′ (Death = c67): the A′ pin FAILs on the B′
#   tree, because Past=Death has left the OPEN set.
# ⚠ Branch-specific: this is kernel-k1's pin, with E10 (2cca4b4) carried. A tree
#   without E10 forms Warm=OP_INTEG and no Compassion pair (track-c's pin).
...
case "$XCOUT" in
  *"| OPEN: Change=OP_COMP See=KAPPA Ongoing=SR_BY None=SR_FROM | PAIRS:"*) : ;;
  *) echo "FAIL  crosscoll: the open cross-population collision set changed — got: $XCOUT"; ok=0 ;;
esac
if [ "$ok" -eq 1 ]; then
    echo "PASS  crosscoll: the 79 lexicon+grammar entries against 4 First Derivations + 21 meta-glyphs + 9 primitives under NORMK — 11 shared-glyph pairs, 7 DECLARED with citations and 4 OPEN pinned exactly (See=KAPPA Change=OP_COMP Ongoing=SR_BY None=SR_FROM), none a ruling"
else
    exit 1
fi
