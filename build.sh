...
say "Sigil: the visual modality — generated glyph forms (LINGUA_ADAMICA.tex)"
# sigil.la is the VISUAL layer of Lingua Adamica: a glyph's form is GENERATED
# from its kappa-decomposition (the SAME Scott-encoded nodes canon.la walks), not
# hand-drawn. Six primitives are DRAWN (their geometry mirrors their signacursion
# sigma); the three derived primitives (SELF/RECOGNITION/LOVE) and every compound
# are GENERATED by SIGIL via the five topological blend modes. A SIGIL is a pure
# r->c->bool predicate over an SZ x SZ grid, built only from integer drawing
# primitives, so a sigil and its ASCII rasterisation are byte-identical on the C
# host and the native VM. The headline autological check: SELF = the self-fold
# MC(BEING) = OVERLAY(BEING)(MIRRORH(BEING)), so its form MUST be horizontally
# symmetric (a form OR-ed with its own mirror), while BEING itself is NOT — which
# proves the fold is real GENERATION, a transform of the parent, not a redraw.
ok=1
# block LABEL FILE -> the 16 grid rows printed under that SHOW label
block () { grep -A16 "$1" "$2" | tail -16; }
# is_hsym: succeed (rc 0) iff every row equals its own reverse (h-mirror symmetry)
is_hsym () { awk '{ r=""; for(i=length($0);i>=1;i--) r=r substr($0,i,1); if(r!=$0) bad=1 } END { exit bad?1:0 }'; }
check_sigil () {  # $1 = engine label, $2 = output file
    grep -q '###' "$2"                || { echo "FAIL  sigil($1): no ink rendered"; ok=0; }
    block "GEN  SELF" "$2" | is_hsym  || { echo "FAIL  sigil($1): SELF=MC(BEING) not horizontally symmetric (self-fold not generated)"; ok=0; }
    if block "ATOM BEING" "$2" | is_hsym; then echo "FAIL  sigil($1): BEING is symmetric (expected asymmetric, so the fold is a real transform)"; ok=0; fi
}
...
if [ "$ok" -eq 1 ]; then
    echo "PASS  sigil: visual modality renders the 6 drawn primitives + GENERATES SELF/RECOGNITION/LOVE and compounds via the 5 blend modes (SELF=MC(BEING) symmetric, BEING not), byte-identical on host and native VM"
else
    exit 1
fi
