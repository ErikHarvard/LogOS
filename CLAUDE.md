...
| `swc_spec.la` / `swc.la` | **Conservative static SWC (well-foundedness) checker** written as a `SPEC` and `GENERATE`d into `swc.la` (regenerated by `build.sh`). Classifies a lambda AST *before* evaluation: **WF** (accept — no bound-var self-application), **ILL** (refuse — eager self-application like `la g. g(g)`, the liar, Ω), or **UNKNOWN** (the undecidable halting residue, e.g. `Z` — let through to the runtime guards). Sound in both directions; never silently mis-classifies. Byte-identical host/VM. See the SWC section below. |
