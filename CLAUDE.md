...
| `canon_spec.la` / `canon.la` | **κ — canonicalization of glyph decompositions** written as a `SPEC` (`import("specpipe.la")`) and `GENERATE`d into `canon.la` (regenerated by `build.sh`, never hand-written). κ (`CANON`) maps a *decomposition* — a primitive leaf or a combination via the five modes ⊗/⊕/▷/⊂/↻ — to a deterministic canonical glyph **spec** (the formal description Theourgia will later render to a sigil; no visual yet). Identity is the **triple bar**: `IS(a)(b) ≡ str_eq(κa)(κb)` — A IS B iff they canonicalize to the *same glyph* (identity, not equality). The **three laws of thought** are theorems over `IS` (each ≡ TRUE). `κ(κ)` is well-defined via κ's self-decomposition `KAPPA`. Byte-identical on host and VM. See the κ section below. |
...
`META_DEBUG`-verified, and compile-time-typed where clean (the logical core —
`TRUE`/`FALSE`/`NOT`/`AND`/`OR`/`IS` and the three laws — carry `:: <type>`
signatures; the Scott-encoded modes, `CANON`, and `KAPPA` are point-free/Z-
recursive and stay trusted). The generated module runs **byte-identically on the
C host and the native VM** (the UTF-8 sigils ⊗⊕▷⊂↻ round-trip through codegen →
SECD). *Honest scope:* this is the canonical-*spec* layer (generation), not
rendering (Theourgia) and not yet a `Seal`/monosemy layer; commutative-mode
normalization and the true phonyms await `LINGUA ADAMICA.tex`.
