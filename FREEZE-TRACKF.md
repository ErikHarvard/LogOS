...
| F37 | HIGH | ★ DIRECTIVE 3 — THE BRANCH COUNT, DERIVED WHERE IT CAN BE (Erik 09-18: "given the current inventory and these operations, the count should be a theorem") | **(1) Under LA's three operations — addition ⊕, merge ⊗, recursion ↻, with NORMK's equivalences — the CLOSURE of any inventory is INFINITE** (⊗ is idempotent for ∃ alone: x, ⊗(x,x), ⊗(x,⊗(x,x)) … stay distinct). So no finite count follows from the operations; a finite count is a RANK — the minimal generating set — which is the codex's OWN audit criterion (App B :4294: 47 mapped, 19 "derivations or sub-interfaces", 28 "necessary and sufficient"). **(2) Computed over branchgenesis.la's inventory (18 + Δ_B, read from the file):** interfaces x in the FORM register (F4 (c)) → **rank 16 of 19**: phonology = ↻(phonetics), sociolinguistics = ⊕(psycholinguistics, psycholinguistics), poetics = ⊗(pragmatics, phonetics) — one derivation per operation; TRUTH register → **15** (+ psycholinguistics = ↻(semantics), by ↻(BEING)≡SELF); register-sets under union → **15** (phonology, discourse, hermeneutics, poetics); branch GLYPHS → **19, by construction** (⊂/▷ heads, which the operations never produce — says nothing). Every minimal generating set is UNIQUE (the irreducibles alone regenerate each derived branch — checked). **(3) The codex's 28 / 36 / 38 are NOT derivable:** its branch formulas are prose (∃ ∩ Structure …), not forms; its audit's 47-list and derivation relation are not in the corpus; Tier 6 is declared open ("new branches emerge"). The count is a theorem ONLY relative to what "derived" means | `derive/branchrank.py` 09-18 (~2.5 min, static); red-pathed: ethics made ⊗(pragmatics, semantics) → FORM rank 15, ethics named | RULING for Erik: which reading defines "derived". Proposed: the INTERFACE form, FORM register — the only reading where the three operations are LA's own acting on forms ⇒ canonical count **16** (3 derived = the codex's "sub-branches or applications"). Then: gate it (static witness pinned at 16, the red path above) and make branchgenesis/branchclosure/crossbranch read ONE list (the engineering fix) |
