...
#     3. cd ~/logos && ./drm_bringup_term.sh
#     4. watch the monitor, in order:
#        a. a GREEN field with a MAGENTA box for ~2s  (startup test pattern)
#        b. a RED window holding white "I AM THAT I AM" on a BLUE desktop
...
SRC=theourgia_text_session_live.la
...
if [ "${FORCE:-0}" != "1" ]; then
  if [ -n "${WAYLAND_DISPLAY:-}${DISPLAY:-}" ] || [[ "$this_tty" != /dev/tty[0-9]* ]]; then
    ...
    echo "  (If you really know better: FORCE=1 ./drm_bringup_term.sh)"
    exit 2
  fi
fi
...
echo
echo "== LIVE SESSION =="
echo "   expect: green/magenta test pattern (2s) -> a red window holding white"
echo "           \"I AM THAT I AM\" on a blue field."
echo "   press the ARROW KEYS to move the whole window (text included) 40px; Ctrl+C to stop."
