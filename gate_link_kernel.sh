...
# ★ THE HONEST LIMIT, AND IT IS NOW ADDRESSED (2026-09-09).
...
# boot.asm here". That is now what it does, PREFERENTIALLY:
#   1. assemble kernel/boot.asm HERE from committed sources — reproducible, and
#      it depends on no other worktree;
#   2. fall back to D's artifact only if that assembly fails.
# ⚠ THE LOCAL BUILD IS A STUB (~7 KB), NOT D's OBJECT (~34 KB). entry.inc is
#   GENERATED by kernel/build_k1.sh and `incbin`s the compiled LA image, so a
#   plain assembly here omits the payload. That is a deliberate trade POROS
...
#   its green cannot be traced. The realistic object remains available —
#   `./kernel/build_k1.sh` produces it in ~7 s — and LOGOS_D_BOOTO still forces it.
...
mkdir -p "$K" || exit 1
# ── prefer a LOCALLY ASSEMBLED object over another worktree's artifact ──
BOOTO=""; BOOTSRC=""
if [ -z "${LOGOS_D_BOOTO:-}" ] && command -v nasm >/dev/null 2>&1 && [ -s kernel/boot.asm ]; then
    if nasm -f elf64 -i .bootfix2_HH2B/ -i kernel/ kernel/boot.asm -o "$K/boot.o" 2>/dev/null \
       && [ -s "$K/boot.o" ]; then
        BOOTO="$K/boot.o"; BOOTSRC="assembled here from kernel/boot.asm (stub: no incbin payload)"
    fi
fi
if [ -z "$BOOTO" ]; then
    BOOTO=${LOGOS_D_BOOTO:-$HOME/logos-d/kernel/boot.o}
    [ -s "$BOOTO" ] || { echo "SKIP  link_kernel: cannot assemble boot.asm here and track D's boot.o is absent ($BOOTO)"; exit 0; }
    cp -p "$BOOTO" "$K/boot.o" || exit 1
    BOOTSRC="track D's prebuilt artifact $BOOTO"
fi
