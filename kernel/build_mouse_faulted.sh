...
#   e_entry --> entry.inc ; boot.asm (default ring-0 path) + incbin --> kernel_mouse_faulted.elf
...
cp kernel/mouse_faulted.la native_input.la
...
echo "[4/4] link -> kernel_mouse_faulted.elf (elf64), repackage container as elf32-i386"
ld -n -T kernel/kernel.ld kernel/boot.o -o kernel/kernel_mouse_faulted_64.elf
objcopy -O elf32-i386 kernel/kernel_mouse_faulted_64.elf kernel/kernel_mouse_faulted.elf

echo "OK: kernel/kernel_mouse_faulted.elf"
