#!/usr/bin/env bash
# LogOS kernel K6c.3a slice — build the ring-3 REAL-LA-image IPC probe ELF.
#   ipc_kernel.la --(native_codegen3, with the new send/recv builtins)--> the LA
...
# Separate output (kernel_k6c3.elf); every other kernel ELF stays byte-identical.
#
...
nasm -f bin native_codegen3_rt.asm -o /tmp/k6c3_rt.bin -l /tmp/k6c3_rt.lst
OFF=$(awk '/ METAL_FLAG: dq/{print $2; exit}' /tmp/k6c3_rt.lst)
[ -n "$OFF" ] || { echo "FAIL K6c3 build: no METAL_FLAG label in rt listing"; exit 1; }
METAL_ABS=$(( 0x400078 + 0x$OFF ))
printf '      METAL_FLAG @ 0x%x (%d)\n' "$METAL_ABS" "$METAL_ABS"

echo "[2/5] compile ipc_kernel.la via native_codegen3 (the LA IPC program)"
cp kernel/ipc_kernel.la native_input.la
...
nasm -f elf64 -dK6C3 -i kernel/ kernel/boot.asm -o kernel/boot_k6c3.o
ld -n -T kernel/kernel.ld kernel/boot_k6c3.o -o kernel/kernel_k6c3_64.elf

echo "[5/5] repackage as elf32-i386 (multiboot1)"
objcopy -O elf32-i386 kernel/kernel_k6c3_64.elf kernel/kernel_k6c3.elf

echo "OK: kernel/kernel_k6c3.elf"
