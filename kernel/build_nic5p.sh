#!/usr/bin/env bash
# LogOS HAL.5p — build the bootable frame-classifying DNS requester (generalises 5l) kernel.
#   nic5p.la --(native_codegen3)--> native_codegen3_out --> kernel_nic5p.elf
...
# at 256 MiB, inside the default 0..1 GiB identity map). 5p adds no builtin —
...
echo "[4/4] link -> kernel_nic5p.elf (elf64 -> elf32-i386 container)"
ld -n -T kernel/kernel.ld kernel/boot.o -o kernel/kernel_nic5p_64.elf
objcopy -O elf32-i386 kernel/kernel_nic5p_64.elf kernel/kernel_nic5p.elf

echo "OK: kernel/kernel_nic5p.elf"
