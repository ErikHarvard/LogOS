...
#   so the high VGA LFB is reachable) + incbin --> kernel_cursor_ctrl.elf
...
cp kernel/cursor_ctrl.la native_input.la
...
echo "[4/4] link -> kernel_cursor_ctrl.elf (elf64 -> elf32-i386 container)"
ld -n -T kernel/kernel.ld kernel/boot.o -o kernel/kernel_cursor_ctrl_64.elf
objcopy -O elf32-i386 kernel/kernel_cursor_ctrl_64.elf kernel/kernel_cursor_ctrl.elf

echo "OK: kernel/kernel_cursor_ctrl.elf"
