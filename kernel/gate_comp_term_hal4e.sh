#!/usr/bin/env bash
# LogOS HAL.4h gate — a TERMINAL WINDOW in the compositor, on the metal.
...
if ! command -v qemu-system-x86_64 >/dev/null 2>&1; then
    echo "SKIP  HAL.4h terminal-window gate: qemu-system-x86_64 not installed"
    exit 0
fi

./kernel/build_comp_term_hal4e.sh >/dev/null 2>&1 || { echo "FAIL  HAL.4h gate: build_comp_term_hal4e.sh failed"; exit 1; }
...
{ for _ in $(seq 1 60); do grep -q 'term ready' "$SERF" 2>/dev/null && break; sleep 0.5; done
  ...
  sleep 2.0
        ...
        -kernel kernel/kernel_comp_term_hal4e.elf -m 512 \
...
for tok in 'term ready' 'term paper=32,32,32' \
    ...
    printf '%s' "$SER" | grep -qF "$tok" || { echo "FAIL  HAL.4h: '$tok' not on serial (rc=$RC, got: $seen)"; ok=0; }
done
