#!/usr/bin/env bash
# LogOS HAL.4h — build the bootable TERMINAL WINDOW in the compositor.
#   comp_term_hal4e.la --(native_codegen3)--> native_codegen3_out --> kernel_comp_term_hal4e.elf
...
echo "[1/4] compile comp_term_hal4e.la"
cp kernel/comp_term_hal4e.la native_input.la
...
echo "[4/4] link -> kernel_comp_term_hal4e.elf"
ld -n -T kernel/kernel.ld kernel/boot.o -o kernel/kernel_comp_term_hal4e_64.elf
objcopy -O elf32-i386 kernel/kernel_comp_term_hal4e_64.elf kernel/kernel_comp_term_hal4e.elf

echo "OK: kernel/kernel_comp_term_hal4e.elf"
