...
#   ata.la --(native_codegen3)--> native_codegen3_out --> kernel_faultprobe2.elf (ring-0)
...
echo "[1/5] compile ata_faultprobe.la via native_codegen3"
cp kernel/ata_faultprobe2.la native_input.la
...
echo "[4/5] link -> kernel_faultprobe2.elf (elf64 -> elf32-i386 container)"
ld -n -T kernel/kernel.ld kernel/boot.o -o kernel/kernel_faultprobe2_64.elf
objcopy -O elf32-i386 kernel/kernel_faultprobe2_64.elf kernel/kernel_faultprobe2.elf
...
echo "OK: kernel/kernel_faultprobe2.elf + kernel/hal3disk.img"
