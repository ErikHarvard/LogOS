#!/usr/bin/env bash
# LogOS HAL.5i — build the bootable IHL-general UDP echo responder kernel.
#   nic5i.la --(native_codegen3)--> native_codegen3_out --> kernel_nic5i.elf
...
# at 256 MiB, inside the default 0..1 GiB identity map). 5i adds no builtin —
# reading the IHL nibble is mod/mul, plain arithmetic.
set -euo pipefail
cd "$(dirname "$0")/.."          # -> ~/logos-d

echo "[1/4] compile nic5i.la via native_codegen3"
cp kernel/nic5i.la native_input.la
...
echo "[4/4] link -> kernel_nic5i.elf (elf64 -> elf32-i386 container)"
ld -n -T kernel/kernel.ld kernel/boot.o -o kernel/kernel_nic5i_64.elf
objcopy -O elf32-i386 kernel/kernel_nic5i_64.elf kernel/kernel_nic5i.elf

echo "OK: kernel/kernel_nic5i.elf"
