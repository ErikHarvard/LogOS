#!/usr/bin/env bash
# LogOS HAL.5g — build the bootable ICMP echo RESPONDER kernel.
#   nic5g.la --(native_codegen3)--> native_codegen3_out --> kernel_nic5g.elf
...
echo "[1/4] compile nic5g.la via native_codegen3"
cp kernel/nic5g.la native_input.la
...
echo "[4/4] link -> kernel_nic5g.elf (elf64 -> elf32-i386 container)"
ld -n -T kernel/kernel.ld kernel/boot.o -o kernel/kernel_nic5g_64.elf
objcopy -O elf32-i386 kernel/kernel_nic5g_64.elf kernel/kernel_nic5g.elf

echo "OK: kernel/kernel_nic5g.elf"
