#!/usr/bin/env bash
# LogOS HAL.4g — build the bootable EDITABLE SCROLLING LINE (comp_edit.la).
#   comp_edit.la --(native_codegen3)--> native_codegen3_out --> kernel_comp_edit.elf
...
echo "[1/4] compile comp_edit.la via native_codegen3 (slow — superlinear codegen)"
cp kernel/comp_edit.la native_input.la
...
echo "[4/4] link -> kernel_comp_edit.elf (elf64 -> elf32-i386 container)"
ld -n -T kernel/kernel.ld kernel/boot.o -o kernel/kernel_comp_edit_64.elf
objcopy -O elf32-i386 kernel/kernel_comp_edit_64.elf kernel/kernel_comp_edit.elf

echo "OK: kernel/kernel_comp_edit.elf"
echo "HAL4G_BUILD_DONE"
