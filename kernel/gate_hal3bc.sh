...
[ "$ok" = 1 ] && echo "PASS  HAL.3bc: the ATA WRITE path's three waits are bounded — a channel that never becomes ready is named on serial WITH THE STAGE THAT STALLED and the kernel exits cleanly, where before it recursed into unmapped memory and died with EXCEPTION 0d. Honest scope: this bounds and DIAGNOSES; it does not re-initialise the channel and retry, and a mid-write stall still leaves the sector unwritten — it just says so." || { echo "HAL.3bc gate RED"; exit 1; }
