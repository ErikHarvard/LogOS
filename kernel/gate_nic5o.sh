#!/usr/bin/env bash
# LogOS HAL.5o gate — the IHL-general ICMP echo REQUESTER (generalises HAL.5c).
#
...
#           OPTION byte as the type); 5o reports icmp=00.
#
...
# from outside. 5o transmits nothing in response: the parse is observable ONLY
...
if ! command -v qemu-system-x86_64 >/dev/null 2>&1; then echo "SKIP  HAL.5o: qemu absent"; exit 0; fi
if ! command -v python3 >/dev/null 2>&1; then echo "SKIP  HAL.5o: python3 absent"; exit 0; fi

./kernel/build_nic5o.sh >/dev/null 2>&1 || { echo "FAIL  HAL.5o gate: build_nic5o.sh failed"; exit 1; }

ok=1

# ---- case 1: the real SLIRP round-trip (IHL=5) ----------------------------
OUT=$(timeout 60 qemu-system-x86_64 \
        -kernel kernel/kernel_nic5o.elf -m 512 \
        -netdev user,id=n0 -device rtl8139,netdev=n0 \
        -serial stdio -display none -no-reboot -no-shutdown 2>/dev/null)
CLEAN=$(printf '%s' "$OUT" | tr -d '\0')
seen=$(printf '%s' "$CLEAN" | tr '\n' ' ' | head -c 300)
for tok in 'nic5o:' 'nic tx ok' 'nic rx ihl=05 et=0800 proto=01 icmp=00' 'nic done'; do
    printf '%s' "$CLEAN" | grep -qF "$tok" \
        || { echo "FAIL  HAL.5o [slirp ihl=5]: '$tok' not on serial (got: $seen)"; ok=0; }
done
...
grep -q "PINGER: listening" "$PLOG" || { echo "FAIL  HAL.5o: harness never bound to $PORT"; ok=0; }
qemu-system-x86_64 -kernel kernel/kernel_nic5o.elf -m 512 \
...
CLEAN=$(tr -d '\0' < "$SER")
seen=$(printf '%s' "$CLEAN" | tr '\n' ' ' | head -c 300)
for tok in 'nic5o:' 'nic tx ok' 'nic rx ihl=06 et=0800 proto=01 icmp=00' 'nic done'; do
    printf '%s' "$CLEAN" | grep -qF "$tok" \
        || { echo "FAIL  HAL.5o [inject ihl=6]: '$tok' not on serial (got: $seen)"; ok=0; }
done
# the harness's own (weaker) witness: it saw the guest transmit at all
if [ "$PRC" -ne 0 ]; then
    echo "FAIL  HAL.5o [inject ihl=6]: harness never saw a frame from the guest ($(tr '\n' ' ' <"$PLOG" | head -c 200))"; ok=0
fi
rm -f "$SER" "$PLOG"

# ---- case 3: ARP NOISE ahead of an INJECTED IHL=6 reply -------------------
# ★ THE NEW CAPABILITY. An ARP broadcast lands in the ring AHEAD of an ICMP echo reply.
# Before HAL.5o this kernel parsed whatever arrived first, so it read the ARP
...
PORT=12452
SER=$(mktemp); PLOG=$(mktemp)
python3 kernel/ping_harness.py arpicmprep6 $PORT 8 >"$PLOG" 2>&1 &
PY=$!
for _ in $(seq 200); do grep -q "PINGER: listening" "$PLOG" && break; sleep 0.05; done
grep -q "PINGER: listening" "$PLOG" || { echo "FAIL  HAL.5o: harness never bound to $PORT"; ok=0; }
qemu-system-x86_64 -kernel kernel/kernel_nic5o.elf -m 512 \
...
CLEAN=$(tr -d '\0' < "$SER")
seen=$(printf '%s' "$CLEAN" | tr '\n' ' ' | head -c 300)
for tok in 'nic5o:' 'nic tx ok' 'nic skip et=0806' 'nic rx ihl=06 et=0800 proto=01 icmp=00' 'nic done'; do
    printf '%s' "$CLEAN" | grep -qF "$tok" \
        || { echo "FAIL  HAL.5o [noise+ihl=6]: '$tok' not on serial (got: $seen)"; ok=0; }
done
...
printf '%s' "$CLEAN" | grep -qF 'ihl=00' \
    && { echo "FAIL  HAL.5o [noise+ihl=6]: 'ihl=00' on serial -- it parsed the ARP as IPv4"; ok=0; }
rm -f "$SER" "$PLOG"

[ "$ok" -eq 1 ] && echo "PASS  HAL.5o: an IHL-GENERAL ICMP echo requester in Lingua Adamica — the same LA kernel read the ICMP type correctly out of BOTH a real SLIRP echo reply (20-byte header) and an injected reply behind a 24-byte header, by READING the IHL nibble (u = 14 + 4*ihl; mod and mul, no bitwise ops) instead of assuming frame offset 34. HAL.5c reports icmp=01 on the second case, reading the first IP option byte as the type. Now also classifies: an ARP broadcast ahead of the reply is skipped on ethertype and the RX ring advanced past it. Single-witness by necessity — this kernel sends no reply, so the parse is observable only on its own serial."
