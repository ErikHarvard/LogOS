#!/usr/bin/env bash
# LogOS HAL.5p gate — the IHL-general DNS requester (generalises HAL.5d).
#
...
#           IP OPTION bytes as the port); 5p reports sport=0035.
#
...
# from outside. 5p transmits nothing in response: the RX parse is observable
...
if ! command -v qemu-system-x86_64 >/dev/null 2>&1; then echo "SKIP  HAL.5p: qemu absent"; exit 0; fi
if ! command -v python3 >/dev/null 2>&1; then echo "SKIP  HAL.5p: python3 absent"; exit 0; fi

./kernel/build_nic5p.sh >/dev/null 2>&1 || { echo "FAIL  HAL.5p gate: build_nic5p.sh failed"; exit 1; }
...
OUT=$(timeout 60 qemu-system-x86_64 \
        -kernel kernel/kernel_nic5p.elf -m 512 \
...
for tok in 'nic5p:' 'nic tx ok' 'nic rx ihl=05 et=0800 proto=11 sport=0035' 'nic done'; do
    printf '%s' "$CLEAN" | grep -qF "$tok" \
        || { echo "FAIL  HAL.5p [slirp ihl=5]: '$tok' not on serial (got: $seen)"; ok=0; }
done
...
grep -q "PINGER: listening" "$PLOG" || { echo "FAIL  HAL.5p: harness never bound to $PORT"; ok=0; }
qemu-system-x86_64 -kernel kernel/kernel_nic5p.elf -m 512 \
...
CLEAN=$(tr -d '\0' < "$SER")
seen=$(printf '%s' "$CLEAN" | tr '\n' ' ' | head -c 300)
for tok in 'nic5p:' 'nic tx ok' 'nic rx ihl=06 et=0800 proto=11 sport=0035 anc=0002' 'nic done'; do
    printf '%s' "$CLEAN" | grep -qF "$tok" \
        || { echo "FAIL  HAL.5p [inject ihl=6]: '$tok' not on serial (got: $seen)"; ok=0; }
done
# the harness's own (weaker) witness: it saw the guest transmit its query
if [ "$PRC" -ne 0 ]; then
    echo "FAIL  HAL.5p [inject ihl=6]: harness never saw a frame from the guest ($(tr '\n' ' ' <"$PLOG" | head -c 200))"; ok=0
fi
rm -f "$SER" "$PLOG"

# ---- case 3: ARP NOISE ahead of an INJECTED IHL=6 reply -------------------
# ★ THE NEW CAPABILITY. An ARP broadcast lands in the ring AHEAD of a DNS response.
# Before HAL.5p this kernel parsed whatever arrived first, so it read the ARP
# frame as IPv4 (ihl=0 -> L4OFF=14, inside the Ethernet header) and never saw
# the reply. It must now classify the ARP out, advance CAPR past it, and read
# the reply from the NEXT ring slot -- while still handling its 24-byte header.
PORT=12453
SER=$(mktemp); PLOG=$(mktemp)
python3 kernel/ping_harness.py arpdnsrep6 $PORT 8 >"$PLOG" 2>&1 &
PY=$!
for _ in $(seq 200); do grep -q "PINGER: listening" "$PLOG" && break; sleep 0.05; done
grep -q "PINGER: listening" "$PLOG" || { echo "FAIL  HAL.5p: harness never bound to $PORT"; ok=0; }
qemu-system-x86_64 -kernel kernel/kernel_nic5p.elf -m 512 \
...
CLEAN=$(tr -d '\0' < "$SER")
seen=$(printf '%s' "$CLEAN" | tr '\n' ' ' | head -c 300)
for tok in 'nic5p:' 'nic tx ok' 'nic skip et=0806' 'nic rx ihl=06 et=0800 proto=11 sport=0035 anc=0002' 'nic done'; do
    printf '%s' "$CLEAN" | grep -qF "$tok" \
        || { echo "FAIL  HAL.5p [noise+ihl=6]: '$tok' not on serial (got: $seen)"; ok=0; }
done
# NEGATIVE: the signature of parsing the ARP as IPv4 -- the exact string the
# pre-classification kernels printed.
printf '%s' "$CLEAN" | grep -qF 'ihl=00' \
    && { echo "FAIL  HAL.5p [noise+ihl=6]: 'ihl=00' on serial -- it parsed the ARP as IPv4"; ok=0; }
rm -f "$SER" "$PLOG"

[ "$ok" -eq 1 ] && echo "PASS  HAL.5p: an IHL-GENERAL DNS requester in Lingua Adamica — the same LA kernel read the UDP source port AND the DNS answer count correctly out of BOTH a real SLIRP DNS response (20-byte header) and an injected response behind a 24-byte header, by READING the IHL nibble (u = 14 + 4*ihl; mod and mul, no bitwise ops) instead of assuming frame offsets 34/35 and 48/49. HAL.5d reports sport=0101 on the second case, reading the IP option bytes as the port. Now also classifies: an ARP broadcast ahead of the reply is skipped on ethertype and the RX ring advanced past it. Single-witness by necessity — this kernel sends no reply, so the RX parse is observable only on its own serial."
