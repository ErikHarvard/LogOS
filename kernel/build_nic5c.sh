#!/usr/bin/env bash
# LogOS HAL.5c — build the bootable ICMP echo round-trip kernel.
#   nic5c.la --(native_codegen3)--> native_codegen3_out --> kernel_nic5c.elf
...
echo "[1/4] compile nic5c.la via native_codegen3"
cp kernel/nic5c.la native_input.la
...
echo "[4/4] link -> kernel_nic5c.elf (elf64 -> elf32-i386 container)"
ld -n -T kernel/kernel.ld kernel/boot.o -o kernel/kernel_nic5c_64.elf
objcopy -O elf32-i386 kernel/kernel_nic5c_64.elf kernel/kernel_nic5c.elf

echo "OK: kernel/kernel_nic5c.elf"
