...
#   ata.la --(native_codegen3)--> native_codegen3_out --> kernel_faultprobe.elf (ring-0)
...
cp kernel/ata_faultprobe.la native_input.la
...
echo "[4/5] link -> kernel_faultprobe.elf (elf64 -> elf32-i386 container)"
ld -n -T kernel/kernel.ld kernel/boot.o -o kernel/kernel_faultprobe_64.elf
objcopy -O elf32-i386 kernel/kernel_faultprobe_64.elf kernel/kernel_faultprobe.elf
...
echo "OK: kernel/kernel_faultprobe.elf + kernel/hal3disk.img"
