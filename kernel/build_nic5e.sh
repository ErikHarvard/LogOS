#!/usr/bin/env bash
# LogOS HAL.5e — build the bootable ARP-responder + ring-advance kernel.
#   nic5e.la --(native_codegen3)--> native_codegen3_out --> kernel_nic5e.elf
...
echo "[1/4] compile nic5e.la via native_codegen3"
cp kernel/nic5e.la native_input.la
...
echo "[4/4] link -> kernel_nic5e.elf (elf64 -> elf32-i386 container)"
ld -n -T kernel/kernel.ld kernel/boot.o -o kernel/kernel_nic5e_64.elf
objcopy -O elf32-i386 kernel/kernel_nic5e_64.elf kernel/kernel_nic5e.elf

echo "OK: kernel/kernel_nic5e.elf"
