...
#   ata.la --(native_codegen3)--> native_codegen3_out --> kernel_hal3_ctrl.elf (ring-0)
...
echo "[1/5] compile ata_ctrl.la (unbounded WAITDRQ — the red-path control) via native_codegen3"
cp kernel/ata_ctrl.la native_input.la
...
echo "[4/5] link -> kernel_hal3_ctrl.elf (elf64 -> elf32-i386 container)"
ld -n -T kernel/kernel.ld kernel/boot.o -o kernel/kernel_hal3_ctrl_64.elf
objcopy -O elf32-i386 kernel/kernel_hal3_ctrl_64.elf kernel/kernel_hal3_ctrl.elf
...
echo "OK: kernel/kernel_hal3_ctrl.elf + kernel/hal3disk.img"
