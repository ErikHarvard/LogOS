...
#   so the high VGA LFB is reachable) + incbin --> kernel_cursor.elf
...
cp kernel/cursor.la native_input.la
...
echo "[4/4] link -> kernel_cursor.elf (elf64 -> elf32-i386 container)"
ld -n -T kernel/kernel.ld kernel/boot.o -o kernel/kernel_cursor_64.elf
objcopy -O elf32-i386 kernel/kernel_cursor_64.elf kernel/kernel_cursor.elf

echo "OK: kernel/kernel_cursor.elf"
